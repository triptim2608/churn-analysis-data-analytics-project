...
- Data Preperation(PYTHON):Used Pandas to combine tables (subscription_id → account_id),Removed duplicates, dealt with missing values, and fixed data types ,Used groupby to combine usage data into one dataset,Use seaborn,coerrelation matrix,and heatmap to visualize data.
...
This project shows that internal product usage metrics don't have a big effect on churn; instead, customer perception and outside factors do.
